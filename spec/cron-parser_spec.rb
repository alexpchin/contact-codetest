...
""")
  ...
  describe "every friday at 11pm" do
    it "returns correct every friday at 11pm" do
...
""")
  ...
  # Using words
  describe "using words" do
    describe "days" do
      it "returns correct every friday at 11pm (using words)" do
        parser = CronParser.new(['0 23 * * fri echo every friday at 11pm'])
        expect(parser.to_s).to eq(
"""
minute: 0
hour: 23
...
""")
      ...
      it "returns correct every monday and friday at 11pm (using words)" do
        parser = CronParser.new(['0 23 * * mon,fri echo every friday at 11pm'])
        expect(parser.to_s).to eq(
"""
minute: 0
hour: 23
...
""")
      ...
      it "returns an error if day was used in wrong position (using words)" do
        parser = CronParser.new(['0 23 * fri * echo every friday at 11pm'])
        expect(parser.to_s).to eq("An error of type ArgumentError happened, message is: 'Incorrect position for day string'")
      end
    end 

    describe "months" do
      it "returns at 00:00 on day-of-month 1 and on Monday in January (using words)" do
        parser = CronParser.new(['0 0 1 jan 1 echo January using words'])
        expect(parser.to_s).to eq(
"""
minute: 0
hour: 0
...
month: 1
day of week: 1
command: echo January using words
""")
      end

      it "returns an error if month was used in wrong position (using words)" do
        parser = CronParser.new(['0 23 jan * * echo every friday at 11pm'])
        expect(parser.to_s).to eq("An error of type ArgumentError happened, message is: 'Incorrect position for month string'")
      end
    end
  end
